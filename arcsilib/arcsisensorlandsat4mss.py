"""
Module that contains the ARCSILandsat4MSSSensor class.
"""
############################################################################
...
#           the pre-processing operations within ARCSI to the landsat 4 MSS
#           datasets.
#
# Author: Pete Bunting
...
class ARCSILandsat4MSSSensor (ARCSIAbstractSensor):
    """
    A class which represents the landsat 4 MSS sensor to read
    ...
    def extractHeaderParameters(self, inputHeader, wktStr):
        ...
        try:
            ...
            # Get the sensor info.
            if ((headerParams["SPACECRAFT_ID"].upper() == "LANDSAT_4") or (headerParams["SPACECRAFT_ID"].upper() == "LANDSAT4")) and (headerParams["SENSOR_ID"].upper() == "MSS"):
                self.sensor = "LS4MSS"
            else:
            ...
            self.b2MaxRad = arcsiUtils.str2Float(headerParams["RADIANCE_MAXIMUM_BAND_2"], 161.900)
            self.b3MinRad = arcsiUtils.str2Float(headerParams["RADIANCE_MINIMUM_BAND_3"], 5.000)
    ...
    def convertImageToRadiance(self, outputPath, outputReflName, outputThermalName, outFormat):
        ...
        bandDefnSeq.append(lsBand(bandName="Green", fileName=self.band1File, bandIndex=1, lMin=self.b1MinRad, lMax=self.b1MaxRad, qCalMin=self.b1CalMin, qCalMax=self.b1CalMax))
        bandDefnSeq.append(lsBand(bandName="Red", fileName=self.band2File, bandIndex=1, lMin=self.b2MinRad, lMax=self.b2MaxRad, qCalMin=self.b2CalMin, qCalMax=self.b2CalMax))
        bandDefnSeq.append(lsBand(bandName="NIR1", fileName=self.band3File, bandIndex=1, lMin=self.b3MinRad, lMax=self.b3MaxRad, qCalMin=self.b3CalMin, qCalMax=self.b3CalMax))
    ...
    def convertImageToTOARefl(self, inputRadImage, outputPath, outputName, outFormat, scaleFactor):
        ...
        solarIrradianceVals.append(IrrVal(irradiance=1827.0))
        solarIrradianceVals.append(IrrVal(irradiance=1569.0))
        solarIrradianceVals.append(IrrVal(irradiance=1260.0))
